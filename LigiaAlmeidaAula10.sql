-- Criar banco de dados da aula 10
create database bd_aula10;
...
CREATE VIEW v_depcidade(nome,cidade)
...
CREATE VIEW v_depcidadehouston (nome, cidade)
...
GROUP BY d.nome
HAVING COUNT(e.cod_empregado) > 0
...
SELECT d.nome AS nome_departamento, COUNT(p.cod_departamento) AS qtd_projetos, COUNT(e.cod_empregado) AS qtd_empregados
FROM tbl_departamentos AS d
LEFT JOIN tbl_projetos AS p ON d.cod_departamento = p.cod_departamento
LEFT JOIN tbl_empregados AS e ON d.cod_departamento = e.cod_departamento
GROUP BY d.nome, d.cod_departamento
HAVING COUNT(p.cod_departamento) > COUNT(e.cod_empregado);

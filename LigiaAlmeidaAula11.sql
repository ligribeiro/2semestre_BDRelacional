-- Criar banco de dados da aula 11
...
CALL proc_upd_nome_depart(5, 'Departamento de Marketing');
...
CREATE OR REPLACE PROCEDURE proc_novoprojeto(IN nome_projeto TEXT, IN codigo_departamento INT)
LANGUAGE SQL
AS $$
    INSERT INTO tbl_projetos (nome, cod_departamento)
    VALUES (nome_projeto, codigo_departamento);
$$;
...
CREATE OR REPLACE PROCEDURE proc_delprojeto(IN codigo_projeto INT)
LANGUAGE SQL
AS $$
    DELETE FROM tbl_projetos
    WHERE cod_projeto = codigo_projeto;
$$;

CALL proc_delprojeto(1);

SELECT * FROM tbl_projetos WHERE cod_projeto = 1;
...
CREATE OR REPLACE PROCEDURE proc_projeto_arquivado(p_cod_projeto INT)
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM tbl_projetos WHERE cod_projeto = p_cod_projeto) THEN
  ...
  RAISE NOTICE 'Projeto arquivado com sucesso.';
END;
$$;

CALL proc_projeto_arquivado(2);

SELECT * FROM tbl_projetos_arquivados WHERE codigo_projeto = 2;

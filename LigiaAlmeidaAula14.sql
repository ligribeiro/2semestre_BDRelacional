-- Criar banco de dados da aula 14
create database bd_aula14;
...
CREATE OR REPLACE FUNCTION atualizar_quantidade_projetos()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION atualizar_quantidade_projetos();

--2 crie uma trigger que atualiza a quantidade de projetos da tbl_departamentos caso um novo projeto for adicionado

CREATE OR REPLACE FUNCTION atualizar_quantidade_projetos_adicionado()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE tbl_departamentos
    SET qtde_projetos = qtde_projetos + 1
...
FOR EACH ROW
EXECUTE FUNCTION atualizar_quantidade_projetos_adicionado();
...
-- a) Criação da tabela tbl_empregado_historico_salario
...
-- b) Criação da trigger para salvar o salário atual
...
-- a) Criação da coluna data_atualizacao na tbl_empregados
ALTER TABLE tbl_empregados
ADD COLUMN data_atualizacao TIMESTAMP;

-- b) Criação da trigger para atualizar a data de atualização
